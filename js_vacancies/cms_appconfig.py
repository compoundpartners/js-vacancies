...
TEMPLATE_PREFIX_CHOICES = getattr(
    settings, 'VACANCIES_TEMPLATE_PREFIXES', [])


@python_2_unicode_compatible
class VacanciesConfig(TranslatableModel, AppHookConfig):
    ...
    paginate_by = models.PositiveIntegerField(
        ...
        blank=False,
        default=5,
        help_text=_('When paginating list views, how many vacancies per page?'),
    )
    ...
    exclude_featured = models.PositiveSmallIntegerField(
        _('Excluded featured vacancies count'),
        blank=True,
        default=0,
        help_text=_(
            'If you are using the Featured vacancies plugin on the vacancy list '
            'view, you may prefer to exclude featured vacancies from the '
            'vacancy list itself to avoid duplicates. To do this, enter the '
            'same number here as in your Featured vacancies plugin.'),
    )
    ...
    search_indexed = models.BooleanField(
        ...
        default=True,
        help_text=_('Include vacancies in search indexes?'),
    )
...
class VacanciesConfigForm(AppDataForm):
    default_published = forms.BooleanField(
        label=_(u'Post published by default'), required=False,
        initial=getattr(settings, 'VACANCIES_DEFAULT_PUBLISHED', False))


setup_config(VacanciesConfigForm, VacanciesConfig)
